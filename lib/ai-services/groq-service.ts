...
const groq = new Groq({
    apiKey: apiKey,
    dangerouslyAllowBrowser: true // Enabling for client-side usage as per current architecture
})
...
function checkApiKey() {
    if (!apiKey) {
        console.warn("NEXT_PUBLIC_GROQ_API_KEY is not set. Using mock data.")
        return false
    }
    return true
}
...
/**
 * Generate structured data using the Groq API
 */
export async function generateStructuredData<T>(
    ...
    try {
        ...
        // For structured data, we want lower temperature and specific system prompt
...
/**
 * Analyze text similarity using embeddings (Mocked for Groq as embeddings API differs)
 * or we can use a simple prompt-based similarity check
 */
export async function analyzeTextSimilarity(text1: string, text2: string): Promise<number> {
    ...
    `
        ...
        const response = await generateWithGroq(prompt, undefined, { temperature: 0.1 })
...
/**
 * Generate a professional summary using Groq AI
 */
export async function generateProfessionalSummary(resumeData: any, jobDescription: string): Promise<string> {
    ...
    `
        ...
        return await generateWithGroq(prompt, undefined, {
            temperature: 0.7,
            maxTokens: 512,
        })
    } catch (error) {
...
/**
 * Generate tailoring tips using Groq AI
 */
...
/**
 * Analyze resume strengths and weaknesses using Groq AI
 */
